...
exports.userCart = async (req, res) => { // console.log(req.body); // {cart: []}
    const {cart} = req.body;
    ...
    let newCart = await new Cart({products, cartTotal, orderdBy: user._id}).save();
...
exports.getUserCart = async (req, res) => {
    ...
    let cart = await Cart.findOne({orderdBy: user._id}).populate("products.product", "_id title price totalAfterDiscount brandId").exec();
...
exports.createOrder = async (req, res) => {
    ...
    let newOrder = await new Order({products, paymentIntent: paymentIntent, orderdBy: user._id}).save();
    ...
    let mailDetails = {
        ...
        text: `Gracias ${user.name} por realizar 
    tu pedido en el marketplace de Cree-Ando,
     tu orden ha sido confirmada y se encuentra
      en proceso de despacho. Puedes preguntar por 
      tu orden usando el código de rastreo ${newOrder._id} en nuestro canal de soporte.`
    };
